...
Be CONSERVATIVE with distance estimates. If uncertain, estimate on the FAR side (safer)."""
    ...
    def turn_towards_object(self, parsed_data: dict) -> str:
        ...
        try:
            ...
            # Try to get horizontal offset with 5 retries
            print(f"🎯 Attempting to center with 5 retries...")
            offset, confidence, centered = self.get_horizontal_offset_with_retry(image_path, target_object, max_retries=5)
            ...
            if centered and centered.lower() == "yes":
                print(f"✅ Target already centered! GPT confirmation: {centered}")
                center_result = f"✅ Already centered (GPT confirmation: {centered})"
            else:
                # Center using proportional control
                center_result = self.center_target_single_attempt(target_object, offset, confidence, centered)
    ...
    def center_target_single_attempt(self, target_object: str, initial_offset: float, confidence: int, centered: str) -> str:
        """Center the target using a single offset measurement with proportional control."""
        try:
            print(f"\n🎯 CENTERING TARGET (SINGLE ATTEMPT)")
            print(f"Initial offset: {initial_offset}, Confidence: {confidence}, Centered: {centered}")
            
            # Check if already centered
            if centered and centered.lower() == "yes":
                return f"✅ Already centered (GPT confirmation: {centered})"
            
            # Calculate proportional turn angle
            # Scale factor: 1.0 means 1.0 offset = 10 degrees turn
            scale_factor = 10.0
            turn_angle = initial_offset * scale_factor
            
            # Clamp turn angle to reasonable range
            turn_angle = max(-15, min(15, turn_angle))  # Clamp between -15 and +15 degrees
            
            print(f"🎯 Calculated turn: {turn_angle:.1f}° (from offset {initial_offset:.3f})")
            ...
            if turn_angle > 0:
                ...
                return "✅ No centering needed (offset is 0)"
            
            print(f"Turn result: {turn_result}")
            return f"✅ Centered with {turn_angle:.1f}° turn: {turn_result}"
