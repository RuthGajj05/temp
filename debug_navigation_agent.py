    ...
    def turn_towards_object(self, parsed_data: dict) -> str:
        ...
        try:
            ...
            if best_view == "left":
                ...
                print("🔄 Object is up, turning left to get better view...")
                turn_result = turn_in_place_left(15, 20, 0.5)  # Small left turn
                return f"✅ Turned left (up view): {turn_result}"
                
            elif best_view == "down":
                print("🔄 Object is down, turning right to get better view...")
                turn_result = turn_in_place_right(15, 20, 0.5)  # Small right turn
                return f"✅ Turned right (down view): {turn_result}"
    ...
    while True:
        try:
            ...
            if command.lower() in ["quit", "exit"]:
                ...
                if len(parts) == 1:
                    # Single word - treat as target
                    target = parts[0]
                    result = agent.debug_navigation(target, turn_towards=True)
                elif len(parts) == 2 and parts[1] == "no-turn":
                    # Target + no-turn
                    target = parts[0]
                    result = agent.debug_navigation(target, turn_towards=False)
                else:
                    print("❌ Invalid command format. Use 'debug target' or 'debug target no-turn'")
                    continue
