...
Be CONSERVATIVE with distance estimates. If uncertain, estimate on the FAR side (safer)."""
    ...
    def turn_towards_object(self, parsed_data: dict) -> str:
        ...
        try:
            ...
            # Try to get centering instructions with 5 retries
            print(f"🎯 Attempting to center with 5 retries...")
            ...
            if turn_instructions is None:
                return f"❌ Failed to get valid response after 5 retries. Initial turn: {turn_result}"
            
            print(f"📊 TURN INSTRUCTIONS: {turn_instructions}")
            
            # Check if already centered
            if turn_instructions.get("centered", False):
                print(f"✅ Target already centered! GPT confirmation: {turn_instructions.get('centered')}")
                center_result = f"✅ Already centered (GPT confirmation: {turn_instructions.get('centered')})"
            else:
                # Center using GPT's specific turn instructions
                center_result = self.center_target_with_instructions(target_object, turn_instructions)
