...
Be CONSERVATIVE with distance estimates. If uncertain, estimate on the FAR side (safer)."""
                ...
                # Check if response looks valid (has our expected fields in strict format)
                has_ranking = "RANKING:" in analysis
                has_best_view = "BEST_VIEW:" in analysis
                has_target = "TARGET:" in analysis
                has_position = "POSITION:" in analysis
                has_distance = "DISTANCE:" in analysis
                has_confidence = "CONFIDENCE:" in analysis
                
                # Check for invalid response patterns (like the one you mentioned)
                invalid_patterns = [
                    "I'm unable to display the specific analysis",
                    "however I can guide you through",
                    "Here's how to approach it",
                    "This method assesses proximity",
                    "while maintaining conservative estimations"
                ]
                
                has_invalid_pattern = any(pattern in analysis for pattern in invalid_patterns)
                
                # All required fields must be present AND no invalid patterns
                if has_ranking and has_best_view and has_target and has_position and has_distance and has_confidence and not has_invalid_pattern:
                    print(f"✅ Vision analysis successful on attempt {attempt + 1}")
                    return analysis
                else:
                    if has_invalid_pattern:
                        print(f"⚠️ Vision response contains invalid pattern (GPT not following format) on attempt {attempt + 1}")
                    else:
                        print(f"⚠️ Vision response incomplete on attempt {attempt + 1}")
                        print(f"Missing fields: ranking={has_ranking}, best_view={has_best_view}, target={has_target}, position={has_position}, distance={has_distance}, confidence={has_confidence}")
    ...
    def turn_towards_object(self, parsed_data: dict) -> str:
        ...
        try:
            ...
            # Center the target until GPT says it's centered
...
Be specific and precise with your instructions. Make sure TURN_DIRECTION matches the POSITION logic above."""
    ...
    def center_target_until_centered(self, target_object: str) -> str:
        """Center the target, continuing until GPT says it's centered."""
        try:
            ...
            while attempt < max_attempts:
                ...
                # Check if already centered - THIS IS THE KEY CHECK
