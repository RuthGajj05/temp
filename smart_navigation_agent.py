...
class SmartNavigationAgent:
    ...
    def _analyze_command(self, command: str) -> str:
        ...
        # Check for "nearest object" patterns (default to 30cm)
        nearest_patterns = [
            r"drive.*?to.*?nearest.*?object",
            r"navigate.*?to.*?nearest.*?object",
            r"go.*?to.*?nearest.*?object",
            r"move.*?to.*?nearest.*?object",
            r"find.*?nearest.*?object",
            r"reach.*?nearest.*?object"
        ]
    ...
    def _handle_distance_navigation(self, command: str) -> str:
        ...
        try:
            # Extract distance from command first (if specified)
            distance_match = re.search(r"(\d+)\s*cm", command)
            if distance_match:
                distance = float(distance_match.group(1))
            else:
                distance = 30.0  # Default distance if not specified
            
            # Check if this is a "nearest object" command
            if re.search(r"nearest.*?object", command):
                target_object = "nearest object"
            else:
                # Extract target object from command
                object_patterns = [
                    r"from\s+(.+?)(?:\s+until|\s+to|\s+at|\s+cm|$)",
                    r"to\s+(.+?)(?:\s+until|\s+to|\s+at|\s+cm|$)",
                    r"near\s+(.+?)(?:\s+until|\s+to|\s+at|\s+cm|$)",
                    r"close\s+to\s+(.+?)(?:\s+until|\s+to|\s+at|\s+cm|$)"
                ]
...
🎯 Distance Navigation (Direct Path):
• "navigate until 30cm from the couch"
• "drive to 20cm from the wall"
• "go until 15cm from the table"
• "move to 25cm from the chair"
• "drive to the nearest object" (defaults to 30cm)
• "drive to the nearest object until 20cm away"
• "navigate to the nearest object until 15cm"
• Note: Stops if obstacles are detected (no avoidance)
...
• Note: Will navigate around obstacles intelligently
