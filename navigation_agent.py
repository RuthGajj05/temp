...
class NavigationAgent:
    ...
    def navigate_to_distance(self, target_distance: float, target_object: str = "object") -> str:
        ...
        try:
            ...
            invalid_readings = 0
            max_invalid_readings = 10  # Stop after 10 consecutive invalid readings
            ...
            while self.navigation_active:
                ...
                try:
                    ...
                    # Validate distance reading (should be positive and reasonable)
                    if current_distance < 0:
                        invalid_readings += 1
                        print(f"⚠️ Invalid distance reading: {current_distance}cm (negative value)")
                        print(f"Invalid readings count: {invalid_readings}/{max_invalid_readings}")
                        
                        if invalid_readings >= max_invalid_readings:
                            self.navigation_active = False
                            return f"❌ Navigation failed: Ultrasonic sensor not working properly. Got {invalid_readings} consecutive invalid readings."
                        
                        print("Skipping this reading and continuing...")
                        time.sleep(0.5)  # Wait a bit before next reading
                        continue
                    elif current_distance > 500:
                        print(f"⚠️ Invalid distance reading: {current_distance}cm (too large)")
                        print("Using 50cm as fallback...")
