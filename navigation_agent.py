...
UP: [target visible? position?]"""
            ...
            if status == "fallback_sonar":
                print("🔄 Vision failed, retrying 5-picture approach...")
                # Try the 5-picture approach one more time
                analysis, retry_status = self.analyze_panorama_with_retries(view_names, image_paths, prompt_text, center_image_path)
                
                if retry_status == "fallback_sonar":
                    print("❌ Vision failed after retry, giving up...")
                    return "TARGET: not visible\nBEST_VIEW: unknown\nCONFIDENCE: 1\nDETAILS: Vision analysis failed after multiple attempts"
                else:
                    print(f"✅ Vision retry successful ({retry_status})")
                    return analysis
...
DETAILS: [additional observations]"""
    ...
    def navigate_to_distance(self, target_distance: float, target_object: str = "object") -> str:
        ...
        try:
            ...
            # Start vision-only navigation loop
            print("🎯 Starting vision-only navigation...")
            vision_failures = 0
            max_vision_failures = 3  # Allow up to 3 vision failures before giving up
            
            while self.navigation_active:
                # Take 5-picture panorama for distance estimation
                print(f"📸 Attempt {self.attempt_count + 1}: Taking 5-picture panorama...")
                vision_analysis = self.analyze_surroundings(target_object, estimate_distance=True, panoramic=True)
                print(f"Vision Analysis: {vision_analysis}")
                
                # Check if vision analysis failed
                if "Vision analysis failed after multiple attempts" in vision_analysis:
                    vision_failures += 1
                    print(f"⚠️ Vision failure {vision_failures}/{max_vision_failures}")
                    
                    if vision_failures >= max_vision_failures:
                        stop_result = stop()
                        self.navigation_active = False
                        return f"❌ Navigation failed: Vision analysis failed after {max_vision_failures} attempts. {stop_result}"
                ...
                # Reset vision failure counter on success
                vision_failures = 0
                
                # Extract distance from vision
                current_distance = self._extract_distance_from_vision(vision_analysis)
                
                if current_distance is not None:
                    print(f"📷 Vision distance estimate: {current_distance}cm")
                    ...
                    print("⚠️ Could not extract distance from vision, using safe fallback")
                    current_distance = 60.0  # Safe fallback distance
                    
                    # Determine movement strategy with fallback distance
                    movement_result = self._determine_movement_strategy(vision_analysis, current_distance, target_distance)
                    print(f"Movement (fallback): {movement_result}")
                
                # Take monitoring photo
