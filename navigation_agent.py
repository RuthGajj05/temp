...
class NavigationAgent:
    ...
    def sonar_sweep_pick_heading(self, arc_deg=180, step_deg=15):
        """
        Rotate in place across a full arc, sample median distance at each heading,
        and return ('left'/'right', total_degrees_to_turn) for the minimum distance.
        """
        import math
        samples = []
        total_angle = 0
        
        print(f"📡 Starting sonar sweep: {arc_deg}° arc in {step_deg}° steps")
        
        # Sweep left first (negative angles)
        for i in range(arc_deg // (2 * step_deg)):
            turn_in_place_left(step_deg, 20, 0.5)
            total_angle -= step_deg
            d = self._get_stable_distance(samples=3)
            if d is not None and d > 0:
                samples.append(('left', total_angle, d))
                print(f"📡 Left {abs(total_angle)}°: {d}cm")
        
        # Sweep right across the full arc (positive angles)
        for i in range(arc_deg // step_deg):
            turn_in_place_right(step_deg, 20, 0.5)
            total_angle += step_deg
            d = self._get_stable_distance(samples=3)
            if d is not None and d > 0:
                samples.append(('right', total_angle, d))
                print(f"📡 Right {total_angle}°: {d}cm")
        
        # Return to center
        if total_angle > 0:
            # We're to the right, turn left back to center
            for _ in range(total_angle // step_deg):
                turn_in_place_left(step_deg, 20, 0.5)
        elif total_angle < 0:
            # We're to the left, turn right back to center
            for _ in range(abs(total_angle) // step_deg):
                turn_in_place_right(step_deg, 20, 0.5)
        ...
        side, angle, distance = min(samples, key=lambda x: x[2])
        print(f"📡 Closest object found: {distance}cm at {angle}° {side}")
        
        return (side, abs(angle))
