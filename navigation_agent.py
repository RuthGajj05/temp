...
DETAILS: [additional observations]"""
    ...
    def _update_target_lock(self, confidence: float, target_position: str, vision_analysis: str) -> tuple[bool, str]:
        """
        Update target lock-on with hysteresis logic.
        ...
        try:
            ...
            # If we have a locked target, check if we should keep it
            if self.locked_target is not None:
                ...
                if confidence < (self.lock_confidence - 2.0):
                    # Confidence dropped significantly - unlock
                    self.locked_target = None
                    self.lock_cycles = 0
                    return False, f"Confidence dropped from {self.lock_confidence:.1f} to {confidence:.1f}"
            ...
            if confidence >= 7.0 and target_visible and target_position != "unknown":
                ...
                self.lock_cycles = 0
                return True, f"Target locked with confidence {confidence:.1f}"
...
Objects detected: {'; '.join(all_objects[:3])}"""
    ...
    def get_navigation_status(self) -> str:
        ...
        try:
            ...
            if self.locked_target is not None:
                status += f"\n🔒 Target locked: {self.locked_target['position']} (confidence: {self.lock_confidence:.1f}, cycles: {self.lock_cycles}/{self.max_lock_cycles})"
            else:
                status += "\n🔓 No target lock"
