...
DETAILS: [additional observations]"""
    ...
    def _determine_movement_strategy(self, vision_analysis: str, current_distance: float, target_distance: float) -> str:
        ...
        try:
            ...
            # Target lock-on with hysteresis logic
            target_locked, lock_reason = self._update_target_lock(confidence, target_position, vision_analysis)
            if target_locked:
                print(f"🔒 Target locked: {self.locked_target} (confidence: {self.lock_confidence:.1f}, cycles: {self.lock_cycles})")
                # Use locked target position instead of current analysis
                target_position = self.locked_target.get('position', target_position)
                confidence = self.lock_confidence
            else:
                print(f"🔓 Target unlocked: {lock_reason}")
            ...
            # Choose movement based on target position (vision) and distance (sonar)
            # Use vision for direction, sonar for step size
            if current_distance is None or current_distance < 0 or current_distance > 500:
                # Invalid sonar - use small, cautious steps
                step_size = 0.3
                speed = int(15 * speed_multiplier)
                print(f"⚠️ Invalid sonar - using cautious steps (speed: {speed}%, duration: {step_size}s)")
            elif current_distance > target_distance + 20:  # Far from target
                step_size = 0.8
            ...
            # Use vision for direction (left/right/center)
            if target_position == "left":
                angle = int(20 * angle_multiplier)
                print(f"🎯 Target on left, turning in place left then moving forward (angle: {angle}°, speed: {speed}%, duration: {step_size}s)")
                ...
                angle = int(20 * angle_multiplier)
                print(f"🎯 Target on right, turning in place right then moving forward (angle: {angle}°, speed: {speed}%, duration: {step_size}s)")
                turn_result = turn_in_place_right(angle, speed, 1.0)
                print(f"Turn result: {turn_result}")
                return move_forward(speed, step_size, check_obstacles=True)
            else:  # center or unknown
                print(f"🎯 Target ahead, moving forward (speed: {speed}%, duration: {step_size}s)")
                return move_forward(speed, step_size, check_obstacles=True)
