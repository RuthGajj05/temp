...
class NavigationAgent:
    ...
    def sonar_sweep_pick_heading(self, arc_deg=120, step_deg=15):
        """
        Rotate in place across an arc, sample median distance at each heading,
        and return ('left'/'right', total_degrees_to_turn) for the minimum distance.
        """
        import math
        samples = []
        # start by sweeping left half
        for _ in range(arc_deg // (2*step_deg)):
            turn_in_place_left(step_deg, 20, 0.5)          # small left turn
            d = self._get_stable_distance(samples=3)       # your existing median read
            if d is not None and d > 0:
                samples.append(('left', len(samples)+1, d))
        # sweep right across the arc
        total_right = 0
        for _ in range(arc_deg // step_deg):
            turn_in_place_right(step_deg, 20, 0.5)
            total_right += step_deg
            d = self._get_stable_distance(samples=3)
            if d is not None and d > 0:
                samples.append(('right', total_right, d))
        # move back left to center (roughly)
        for _ in range((arc_deg // (2*step_deg))):
        ...
        side, ticks, _d = min(samples, key=lambda x: x[2])
        return (side, ticks*step_deg)
