...
DETAILS: [additional observations]"""
    ...
    def _determine_movement_strategy(self, vision_analysis: str, current_distance: float, target_distance: float) -> str:
        """Determine movement strategy - just move forward toward the centered target."""
        try:
            # Extract confidence level
            confidence = self._extract_confidence_level(vision_analysis)
            print(f"🎯 Vision confidence: {confidence}/10")
            
            # Check if target is still visible and centered
            target_position = self._extract_target_position(vision_analysis.lower())
            if target_position != "center" or confidence < 4:
                print("⚠️ Target not centered or low confidence, stopping to re-center...")
                return "STOP_FOR_RECENTERING"
            ...
            print(f"🎯 Moving forward toward centered target (speed: {speed}%, duration: {step_size}s)")
    ...
    def navigate_to_distance(self, target_distance: float, target_object: str = "object") -> str:
        ...
        try:
            ...
            while self.navigation_active:
                ...
                # Check if we need to re-center the target
                if movement_result == "STOP_FOR_RECENTERING":
                    print("🔄 Target lost center, re-centering...")
                    # Take a single photo to check current position
                    recenter_photo = capture_image(f"recenter_check_{self.attempt_count}.jpg")
                    if "successfully" in recenter_photo.lower():
                        recenter_analysis = self.analyze_surroundings(target_object, estimate_distance=False, panoramic=False)
                        target_position = self._extract_target_position(recenter_analysis.lower())
                        
                        if target_position == "left":
                            print("🔄 Target on left, turning left to re-center...")
                            turn_result = turn_in_place_left(20, 20, 0.8)
                            print(f"Turn result: {turn_result}")
                        elif target_position == "right":
                            print("🔄 Target on right, turning right to re-center...")
                            turn_result = turn_in_place_right(20, 20, 0.8)
                            print(f"Turn result: {turn_result}")
                        else:
                            print("🔄 Target position unclear, making small adjustment...")
                    ...
                    # Skip this movement cycle and try again next time
