...
def check_obstacle_safe(distance_threshold: float = 15.0) -> bool:
    ...
    try:
        ...
        # Handle invalid readings (negative, None, or very large values)
        if distance is None or distance < 0 or distance > 500:
            return True  # Treat invalid readings as safe (no obstacle)
            ...
            if check_obstacles:
                ...
                while time.time() - start_time < duration:
                    if not check_obstacle_safe():
                        # Get the actual distance reading for the error message
                        try:
                            actual_distance = px.ultrasonic.read()
                            if actual_distance is None or actual_distance < 0 or actual_distance > 500:
                                # Invalid reading - continue movement (camera will handle navigation)
                                time.sleep(0.05)
                                continue
                            else:
                                px.stop()
                            ...
                            # Sensor error - continue movement
                            time.sleep(0.05)
                            continue
                    time.sleep(0.05)
            else:
...
def turn_in_place_left(angle: float, speed: int, duration: Optional[float] = None) -> str:
    """Turn left in place using steering servo and differential motor control."""
    global _servo_angles, _continuous_movement
    ...
    try:
        ...
        # Set steering servo to left turn angle for better turning
        steering_angle = -30  # Turn steering left for left turn
        ...
        if _continuous_movement:
            ...
            time.sleep(duration)
            px.stop()
            # Reset motor calibration and steering to normal
            px.cali_dir_value = [1, 1]
            px.set_dir_servo_angle(0)
            _servo_angles['dir_servo'] = 0
        ...
        # Ensure we reset calibration and steering even on error
        try:
            px = get_picarx()
            px.cali_dir_value = [1, 1]
            px.set_dir_servo_angle(0)
...
def turn_in_place_right(angle: float, speed: int, duration: Optional[float] = None) -> str:
    """Turn right in place using steering servo and differential motor control."""
    global _servo_angles, _continuous_movement
    ...
    try:
        ...
        # Set steering servo to right turn angle for better turning
        steering_angle = 30  # Turn steering right for right turn
        px.set_dir_servo_angle(steering_angle)
        _servo_angles['dir_servo'] = steering_angle
